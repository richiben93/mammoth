...
plt.ioff()
os.makedirs('figs99', exist_ok=True)
stds, kms, OO, bbs = {}, {}, {}, {}
wcons = []
for dir in tqdm(dirs):#os.listdir(homedir)):
    #bufeats
    if os.path.isdir(os.path.join(homedir, dir)) and 'testfeats.pkl' in os.listdir(os.path.join(homedir, dir)):
        try:
            all_data = pickle.load(open(os.path.join(homedir, dir, 'testfeats.pkl'), 'rb'))
        except:
        ...
        plt.savefig(f'figs99/{"_".join([str(x) for x in list(all_data.keys())[0]])}_{dir}.pdf')
        plt.close()
