...
class ErBsDL(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        super(ErBsDL, self).__init__(backbone, loss, args, transform)
    ...
    def begin_task(self, dataset):
        ...
        if self.args.reorder_futures and self.args.futures > 0 and self.task > 0:
            ...
            means = torch.zeros((self.classes, self.args.futures), device=outputs.device, dtype=outputs.dtype)
            for i in range(self.classes):
                means[i] = outputs[labels == i].mean(dim=0)
            avgs = means.mean(dim=0)
            means = means - avgs

            best_assignment = linear_sum_assignment(means.cpu().numpy(), maximize=True)[1].tolist()
