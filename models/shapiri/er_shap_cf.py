...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Continual learning via'
                                        ' Experience Replay with Shapiro and future heads in cross-entropy.')
...
class ErShapCf(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        super(ErShapCf, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        eye_mask = eye[task_labels]
        if self.task+1 < self.n_task:
            eye_mask += eye[(self.task+1)].repeat((inputs.shape[0], 1))
        mask = torch.repeat_interleave(eye_mask, self.classes, 1)
