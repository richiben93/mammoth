...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Continual learning via'
                                        ' Experience Replay with BetaShaping and Progressive Learning Rate.')
    ...
    parser.add_argument('--lr_iters', type=int, default=5,
                        help='lr scheduler iters')
    return parser


class ErBsPlr(ContinualModel):
    NAME = 'er_bs_plr'
    COMPATIBILITY = [
        'class-il',
        # 'domain-il',
    ...
    def __init__(self, backbone, loss, args, transform):
        super(ErBsPlr, self).__init__(backbone, loss, args, transform)
        ...
        # self.scheduler = lr_scheduler.LinearLR(self.opt)
        # wandb.init(project="batch-shaping", entity="ema-frasca", name="er_bs: progressive lr", config=vars(self.args))
    ...
    def begin_task(self, dataset):
        self.scheduler = lr_scheduler.LinearLR(self.opt, start_factor=0.1, total_iters=self.args.lr_iters)
