...
class XDerRPCEgap(EgapModel):
    NAME = 'xder_rpc_egap'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(XDerRPCEgap, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        loss_egap = torch.tensor(0.)
        if self.task > 0 and self.args.rep_minibatch > 0 and self.args.replay_weight > 0:
            loss_egap = self.get_replay_loss()
            self.wb_log['egap_loss'] = loss_egap.item()
            loss_egap *= self.args.replay_weight

        loss = loss_stream + loss_der + loss_derpp + loss_constr_futu + loss_constr_past + loss_egap
