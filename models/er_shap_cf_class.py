...
class ErShapCfClass(ContinualModel):
    NAME = 'er_shap_cf_class'
    COMPATIBILITY = [
        'class-il',
        # 'domain-il',
        # 'task-il',
    ...
    def __init__(self, backbone, loss, args, transform):
        super(ErShapCfClass, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if self.task+1 < self.n_task:
            masked_outputs = outputs[:, :((self.task + 2) * self.classes)]
        else:
